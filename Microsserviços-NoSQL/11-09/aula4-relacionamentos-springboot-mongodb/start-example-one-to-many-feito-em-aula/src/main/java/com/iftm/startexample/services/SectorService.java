...
public class SectorService {
    ...
    public ResponseEntity<SectorDTO> addEmployeById(ObjectId sectorId, ObjectId employeeId) {
        if(sectorId == null || employeeId == null)
            return ResponseEntity.badRequest().build();
        ...
        if(dbSector.get().getEmployees() == null) {
            List<Employee> employees = new ArrayList<>();
            dbSector.get().setEmployees(employees);
        }
        dbSector.get().getEmployees().add(dbEmployee.get());
        return ResponseEntity.ok(new SectorDTO(repository.save(dbSector.get())));
    }
